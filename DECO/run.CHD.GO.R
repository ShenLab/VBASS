...
print(gs.id)
if (!file.exists(paste0('CHD.GO/', gs.id, '.RDS'))) {
  xInputGS.raw <- read.csv("~/ld1/VBASS_xTADA_paper/scalar_VBASS/result/table.S4.csv", header = TRUE, as.is = TRUE)
  xInputGS <- xInputGS.raw[,c("Gene", "dn_LGD", "dn_Dmis", "mut_LGD", "mut_Dmis", "covariates_reordered")]
  ...
  xInputGS$GS <- as.numeric(xInputGS.raw$HGNC %in% go.terms$genesets[[to.select[gs.id]]])
  ...
  saveRDS(out, file = paste0('CHD.GO/', gs.id, '.RDS'))
}
