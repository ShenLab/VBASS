...
all_dnv_table <- c()
for (i in 1:50) {
  train.input <- read.csv(paste0('AutoEncoderTADA.input.simulation.realexp/input.x.',
                                 i, '.csv'),
                          row.names = 1)
  train.input.var <- read.csv(paste0('AutoEncoderTADA.input.simulation.realexp/input.x_var.',
                                     i, '.csv'),
                              row.names = 1)
  ...
  real_BF <- read.csv(paste0('AutoEncoderTADA.input.simulation.realexp/real.log.BF.',
                             i, '.csv'),
                      row.names = 1)
  ...
  TADA.result <- readRDS(paste0('AutoEncoderTADA.input.simulation.realexp/extTADA.simulation.realexp.',
                                i, '.RDS'))
  TADA_dnv_table <- TADA.result$dataFDR.full.posterior
  dnv_table$TADA_qvalue <- TADA_dnv_table$qvalue[match(rownames(dnv_table),
                                                       TADA_dnv_table$Gene)]
  # PPA of dnv_table
  true_labels <- read.csv(paste0('AutoEncoderTADA.input.simulation.realexp/test.label.',
                                 i, '.csv'))
  training_true_labels <- read.csv(paste0('AutoEncoderTADA.input.simulation.realexp/input.label.',
                                          i, '.csv'))
...
FDR_curve <- data.frame(qvalue=c(FDR_curve_1$qvalue, FDR_curve_2$qvalue),
                        FDR=c(FDR_curve_1$FDR, FDR_curve_2$FDR),
...
ggsave(plot = p, filename = paste0(result.folder, 'fig.4.A.pdf'),
       width = 5, height = 4)
...
p<-ggplot(to.plot, aes(x=recall,y=precision,col=model)) +
  ...
  ylim(0, 1) +
  theme_light()
ggsave(plot = p, filename = paste0(result.folder, 'fig.4.B.pdf'),
       width = 5, height = 4)
