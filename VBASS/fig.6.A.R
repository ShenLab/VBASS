...
TADA.result <- readRDS('SPARK_extTADA.WES1.RDS')
...
threshold_1 <- 0.05
threshold_2 <- 0.05
dnv_table$group[dnv_table$qvalue<=threshold_1 & dnv_table$TADA_qvalue<=threshold_2] = 'both'
dnv_table$group[dnv_table$qvalue>threshold_1 & dnv_table$TADA_qvalue<=threshold_2] = 'TADA_only'
dnv_table$group[dnv_table$qvalue<=threshold_1 & dnv_table$TADA_qvalue>threshold_2] = 'VBASS_only'
dnv_table$group[dnv_table$qvalue>threshold_1 & dnv_table$TADA_qvalue>threshold_2] = 'NA'
dnv_table$label <- rownames(dnv_table)
dnv_table$label[dnv_table$group %in% c('NA', 'both')] <- ''
# plot comparison of log_BF and TADA_qvalue

p <- ggplot(dnv_table, aes(x=qvalue, y=TADA_qvalue, col=group, label=label)) +
  geom_point() +
