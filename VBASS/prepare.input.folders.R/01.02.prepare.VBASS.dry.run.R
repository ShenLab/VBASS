TADA.output <- readRDS('data/SPARK_extTADA.WES1.RDS')
dir.create('VBASS.input.dry.run/')
...
# for dry run, we don't input any expression
genes.zhong_2018[1:dim(genes.zhong_2018)[1], 1:dim(genes.zhong_2018)[2]] <- 0 
genes.lamanno_2016[1:dim(genes.lamanno_2016)[1], 1:dim(genes.lamanno_2016)[2]] <- 0 

genes.x <- cbind(genes.zhong_2018, genes.lamanno_2016, genes.dnv)
rownames(genes.x) <- genes
write.csv(genes.x, file = 'VBASS.input.dry.run/input.x.csv')
# begin x.var
...
write.csv(genes.x_var, file = 'VBASS.input.dry.run/input.x_var.csv')
# begin gene.label
# library(dplyr)
...
write.csv(genes.label, file = 'VBASS.input.dry.run/input.label.csv')

# write.csv(label.file.testing, file = "VBASS.input.dry.run/testing.label.csv")
