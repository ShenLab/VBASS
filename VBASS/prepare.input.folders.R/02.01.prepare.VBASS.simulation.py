...
data_folder = '../VBASS.input/'
X = pd.read_csv(f'{data_folder}/score.csv', index_col=0)
...
labels = pd.read_csv(f'{data_folder}/label.csv')
y = labels['conditions']
y[y=='SPARK.risk'] = 0
y[y=='control.LGD'] = 1
y = y.astype(int)
...
with open(f'{data_folder}/elastic.net.report.txt', 'w') as outputfile:
...
import pickle
with open(f'{data_folder}/elasticnet.pkl', 'wb') as outputfile:
...
prediction.to_csv(f'{data_folder}/prediction.csv')

all_X = pd.read_csv(f'{data_folder}/all.scores.csv', index_col=0)
all_prediction = pd.DataFrame(elastic_net_classifier.predict_proba(all_X), index=all_X.index)
all_prediction.to_csv(f'{data_folder}/all.prediction.csv')
# PC_loadings = pd.read_csv(f'{data_folder}/PC.loadings.csv', index_col=0)
...
elastic_net_loadings.to_csv(f'{data_folder}/elastic.net.loadings.csv')
elastic_net_intercept = pd.DataFrame(elastic_net_classifier.intercept_)
elastic_net_intercept.to_csv(f'{data_folder}/elastic.net.intercept.csv')
