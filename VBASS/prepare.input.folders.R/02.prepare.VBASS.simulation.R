...
simulate_AutoEncoder <- function(effect_size_mean = c(16.6, 6.8),
  ...
  set.seed(seed)
  output.folder <- paste0('VBASS.simulation.with.RDS/')
  dir.create(output.folder)
  # first simulate expression data
  genes.x <- read.csv('VBASS.input/input.x.csv', row.names = 1)
  genes.x_var <- read.csv('VBASS.input/input.x_var.csv', row.names = 1)
  ...
  real.pi <- read.csv('VBASS.input/all.prediction.csv', row.names = 1)
