...
# reorder by rows
training.genes.sc <- training.genes.sc[order(numeric.row.anno$log_pi, decreasing=TRUE),]
gene.labels <- gene.labels[order(numeric.row.anno$log_pi, decreasing=TRUE),]
numeric.row.anno <- data.frame(log_pi=log(as.numeric(pi$pi[match(rownames(gene.labels), pi$HGNC)])))
plot.complex.heatmap(training.genes.sc, row.anno = gene.labels, 
                     numeric.row.anno = numeric.row.anno,
                     col.anno = col.anno, title = 'Expression', cluster_rows = F,
                     width = 10, height = 15, col_font_size = 6, row_dend_reorder=FALSE,
                     savefilename = 'figs/sup.fig.4.A.pdf')
plot.complex.heatmap(training.genes.sc, row.anno = gene.labels, 
                     numeric.row.anno = numeric.row.anno,
