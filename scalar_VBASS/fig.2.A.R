...
if (is.na(seed)) {
  for (i in 1:100) {
    ...
    table$scalarVBASS_qvalue <- table$qvalue
...
# precision recall
table <- table[order(table$scalarVBASS_qvalue), ]
all.true <- sum(table$true)
scalarVBASS.pc <- data.frame(qvalue=table$scalarVBASS_qvalue,
                       true=table$true,
                       precision=table$true,
                       recall=table$true)
scalarVBASS.pc <- scalarVBASS.pc[scalarVBASS.pc$qvalue<=0.2, ]
...
for (i in 1:dim(scalarVBASS.pc)[1]) {
  scalarVBASS.pc$precision[i] <- sum(scalarVBASS.pc$true[1:i])/i
  scalarVBASS.pc$recall[i] <- sum(scalarVBASS.pc$true[1:i])/all.true
}
...
to.plot <- data.frame(precision=c(scalarVBASS.pc$precision, extTADA.pc$precision, poisson.pc$precision),
                      recall=c(scalarVBASS.pc$recall, extTADA.pc$recall, poisson.pc$recall),
                      model=c(rep('VBASS', dim(scalarVBASS.pc)[1]), 
...
write.csv(to.plot, file = paste0(result.folder, 'fig.', samplesize, '/c.', C, '.size.', samplesize, '.precision.recall.csv'))
