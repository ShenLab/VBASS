estimatePars <- function(pars, mcmcResult, nThin = NULL){
    ...
    pars <- pars[grep("hyper|pi|A|B", pars)]
    message("This is the parameter estimation steps in extTADA that estimate parameters based on mode\n",
            "We have another parameter estimation step based on whole posterior distribution\n",
            "Check calculateFDR.full.posterior.R for details\n",
    ...
    for (iPar in pars) {
        ...
        if ((sd(xData) > 10^-3) & (length(grep("Beta|A|B", iPar)) == 0))
