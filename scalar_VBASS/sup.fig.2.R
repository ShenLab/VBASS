...
if (is.na(seed)) {
  for (i in 1:100) {
    ...
    #                            samplesize, ".seed.", i, ".RDS"))
    xTADAsim_post <- readRDS(paste0("RDS.files/scalarVBASS_simulation_c.", C ,".size.",
                                    samplesize, ".seed.", i, ".posterior.RDS"))
...
FDR_local_curve <- data.frame(FDR=c(FDR_local_curve_1$qvalue, FDR_local_curve_2$qvalue, FDR_local_curve_3$qvalue),
                        ...
                        model=c(rep('VBASS', dim(FDR_local_curve_1)[1]),
...
ggsave(plot = p, filename = paste0(result.folder, 'fig.', samplesize, '/c.', C, '.size.', samplesize, '.compare.PP.FDR.local.pdf'),
       width = 4.5, height = 3)
write.csv(FDR_local_curve, filename = paste0(result.folder, 'fig.', samplesize, '/c.', C, '.size.', samplesize, '.compare.PP.FDR.local.csv'))
