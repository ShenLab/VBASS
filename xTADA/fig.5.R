xTADA_result <- readRDS('result/CHD_xTADA_expression.RDS')
...
dnv_table$group[dnv_table$qvalue>threshold_1 & dnv_table$extTADA.qvalue<=threshold_2] = 'extTADA_only'
dnv_table$group[dnv_table$qvalue<=threshold_1 & dnv_table$extTADA.qvalue>threshold_2] = 'xTADA_only'
dnv_table$group[dnv_table$qvalue>threshold_1 & dnv_table$extTADA.qvalue>threshold_2] = 'NA'
dnv_table$label <- dnv_table$HGNC
dnv_table$label[dnv_table$group %in% c('NA', 'both')] <- ''
library(ggplot2)
library(ggrepel)
p <- ggplot(dnv_table, aes(x=qvalue, y=extTADA.qvalue, col=group, label=label)) +
  geom_point() +
